...
ari<-read.table('clipboard')
ari.hel <- decostand(ari, "hellinger") ## Hellinger-transform the species dataset

geo<-read.table('clipboard')

sue<-read.table('clipboard')

cli<-read.table('clipboard')

###Forward selection for each data set 
  ##EnvChem selection

    rda1chem<-rda(ari.hel~1,geo)###model with intercept only
    rda2chem<-rda(spe.hel~.,sue)###model with all explanatory variables
    rda2chem<-rda(spe.hel~.,cli)###model with all explanatory variables
