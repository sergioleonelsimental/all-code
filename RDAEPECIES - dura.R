...
###Forward selection for each data set 
  ...
  ##Envclima selection
    ...
    rdasenvcli<-rda(spe.hel~ sprp,data=envcli)
    ...
    rdasenvcli<-rda(spe.hel~ Latitud,data=envcli)
...
###### Parsimonious subsets of explanatory variables (based on 
  ...
  names(envcli)##alt
  envcli.pars <- envcli[, c(19)]
  
  names(envgeo)##alt
  envgeo.pars <- envgeo[, c(2)]
    
# Variation partitioning
spe.part <- varpart(spe.hel, envcli.pars, envgeo.pars)
spe.part
