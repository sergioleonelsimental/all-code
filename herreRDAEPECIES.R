...
###Forward selection for each data set 
  ...
  ##Envclima selection
    ...
    rdasenvcli<-rda(spe.hel~ sday + AAI + smrpb ,data=envcli)
...
spe.part
plot(spe.part, digits=2) ; text(40, 27, "Aquí otro",col = "red")
...
env.pars <- cbind(envcli.pars, envsue.pars, envgeo.pars )
anova.cca(rda(spe.hel, env.pars), step=1000)

env1.pars <- cbind(envsue.pars, envcli.pars )
anova.cca(rda(spe.hel, env1.pars), step=1000)

env2.pars <- cbind(envsue.pars, envgeo.pars )
anova.cca(rda(spe.hel, env2.pars), step=1000)

env3.pars <- cbind(envcli.pars, envgeo.pars )
anova.cca(rda(spe.hel, env3.pars), step=1000)



# Test of fraction [a]
anova.cca(rda(spe.hel, envcli.pars, envgeo.pars), step=1000)

# Test of fraction [b]

anova.cca(rda(spe.hel,envsue.pars,envgeo.pars ), step=1000)
# Test of fraction [c]

anova.cca(rda(spe.hel, envgeo.pars, envcli.pars), step=1000)
